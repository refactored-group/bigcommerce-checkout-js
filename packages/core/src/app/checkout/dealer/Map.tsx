...
export class MapContainer extends Component<IProps, IState> {
  ...
  onMarkerClick = (props: any, marker: any) => {
    const formattedDealerPhoneNumber = formatPhoneNumber({
      phoneNumber: props.dealer.phone_number,
    });
  ...
  render() {
    return (
      ...
      >
        {this.props.dealers.map((dealer: any, index: any) => this.renderMarker(dealer, index))}
          ...
          <React.Fragment>
            {this.state.activeDealer && (
              <div className="info-window-content" title="Preferred Dealer">
                {this.state.activeDealer.preferred && (
                  <div className="locator-modal-dealer-star"></div>
                )}
                <h4>{this.state.activeDealer.business_name}</h4>
                <h5>{this.state.activeDealer.premise_street}</h5>
                <a href={`tel:${this.state.activeDealerPhone}`}>
                  {this.state.activeDealerPhoneFormatted}
                </a>
